 ...
 ***************/
  ...
  const addLeaderBoardButtons = (page, numResults, resPerPage = 100) => {
	...
	document.querySelector('.leaderboards').insertAdjacentHTML('afterend', markup);	
  }
  ...
  const removeHighlightedBtns = () => {
  	const btns = document.querySelector('.leaderboard-options').getElementsByTagName('li');
...
const renderHistoryCell = (match, strings) => {
	...
	//add to ui
	document.querySelector('.matchHistory').getElementsByTagName('tbody')[0].insertAdjacentHTML('beforeend', markup);
}
...
const generateTables = (game_players) => {
	...
	let team1 = [], team2 = [], team3= [], team4 = [];	 //arrays holding data of team1, team2
	let one = false, two = false, three = false, four= false; //flags indicating if the team number is used
	...
	team.forEach( cur => {
		...
		markup += `
			<tr>
				...
				<td> ${team[x].rating}</td>
				<td> ${team[x].name}</td>
				${(team[x].won === true && team[x].won !== null) ? '<td> <i class="fas fa-crown"></i><td>': ''}
			</tr>
		`;
		x++;
});
...
const loadDefaultTable = async () => {
	...
	addLeaderBoardButtons(state.curPage, state.AOELadder.ladder.leaderboard.length);
}
...
document.querySelector('.leaderboards').addEventListener('click', async e => {
 		...
 		if(e.target.matches('.player')){
 			...
 			const rank = e.target.parentNode.parentNode.querySelector(".player-rank").innerHTML;
			...
			state.matchHistory = await state.history.getPlayerMatchHistory(state.pid);
 			...
 			generateMatchHistory(state.matchHistory.matchHistory, state.matchHistory.strings);
...
//add event listner to close button of match history
 		document.querySelector('.close-match-history').addEventListener('click', e => {
...
window.addEventListener('click', e => {
	...
	if(e.target.matches('.btns-footer')){
 			...
 			//go to the last page of players
 				...
 				state.curPage = Math.ceil(state.AOELadder.ladder.leaderboard.length/100);
 				updateLeaderBoard(state.AOELadder.ladder.leaderboard, state.curPage);
...
const el = document.querySelector('.leaderboard-options').getElementsByTagName('a');
...
document.querySelector('.find').addEventListener('click', (e)=>{
	...
	if(searchBox.value !== ""){
	const index = searchPlayerByName(searchBox.value, state.AOELadder.ladder.leaderboard);
